...
def _parepare_school_data(df: pd.DataFrame, subject: str) -> None:
    ...
    # remove white space in postcode
    df[COLUMN_SCHOOL_POSTCODE] = df[COLUMN_SCHOOL_POSTCODE].str.replace(" ", "")
...
def _parepare_student_data(df: pd.DataFrame) -> None:
    ...
    # remove white space in postcode
    df[COLUMN_STUDENT_POSTCODE] = df[COLUMN_STUDENT_POSTCODE].str.replace(" ", "")
